...
const ActionResult: React.FC<ActionResultProps> = ({ title, score, total, onStartNew, buttonText, wrongAnswers, isModal = true }) => {
  ...
  return (
    <div className={containerClass}>
      ...
      <p className="text-3xl font-bold mb-6 text-center">Your score: <span className="text-blue-400">{score}</span> / {total}</p>
          ...
          <ul className="space-y-4">
            {wrongAnswers.map((answer, index) => (
              <li key={index} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <p className="text-blue-400 font-medium">Correct answer: {answer.correctAnswer}</p>
              </li>
            ))}
          </ul>
        </div>
